class VoteManagementSerializer < BaseSerializer
  attributes :id, :auto_generate_id, :created_date, :created_by, :association_id, :association_name, :meeting_type, :category, :ratification_type, :title, :approval_due_date, :is_voting_open, :status, :start_date_time, :end_date_time
  ...
  attribute :voting_data do |object|
    ...
    # {total_vote: total, approved: approved, approved_percentage: approved_percentage, rejected: rejected, rejected_percentage: rejected_percentage} rescue nil
    object.voting_data
  end
