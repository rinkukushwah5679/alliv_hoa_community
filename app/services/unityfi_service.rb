...
class UnityfiService
	...
	def generate_token
		begin
	        ...
	        unityfi.update(
	          ...
	          token_expires_at: Time.now + 30.minutes
	        )
	      else
	        # create new record
	...
	def create_location_user(assocation)
		...
		if unityfi[:error].present?
			Rails.logger.info "\e[31m=============Error from Unityfi when generate token for create location: #{unityfi[:error]}==============\e[0m"
			return
		end
	...
	def create_funding_account(location_user, bank)
		unityfi = generate_token
		if unityfi[:error].present?
			Rails.logger.info "\e[31m=============Error from Unityfi when generate token for create funding account like bank: #{unityfi[:error]}==============\e[0m"
			return
		end
