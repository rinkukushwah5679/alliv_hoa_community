class WorkOrder < ApplicationRecord
  ...
  enum :requestor_type, { "Resident" => "unit_owner", "Association" => "association", "Other" => "other"}
  ...
  def self.data_requestor_type(user_id)
    ...
    return ["BoardMember"] if user.present? && user.current_role == "BoardMember"
    return ["Resident", "Association", "Other"]
  end
