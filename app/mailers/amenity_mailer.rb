class AmenityMailer < ApplicationMailer
	...
	def notify_admins_and_managers_when_booking_amenity(user, reservation, association)
		@user = user
    ...
    subject = "New amenity reservation in #{@association&.name&.titleize}"


    template_path = Rails.root.join(
      "app", "views", "amenity_mailer", "notify_admins_and_managers_when_booking_amenity.html.erb"
    )

    erb_template = File.read(template_path)
    html_body = ERB.new(erb_template).result(binding)
    plain_text = ActionView::Base.full_sanitizer.sanitize(html_body)
    Notification.create(user_id: @user.id, notifiable: reservation, title: subject, message: plain_text, delivery_methods: ["email", "portal"], created_by: reservation.created_by, updated_by: reservation.updated_by)
    mail(
      to: @user.email,
      subject: subject,
    ) do |format|
